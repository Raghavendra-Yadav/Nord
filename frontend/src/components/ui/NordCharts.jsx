// NordCharts — GradientBarReport, SmoothedArea, Donut, SparkBars, MultilineChart
...
export function GradientBarReport({ data, highlightIdx = 0 }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" preserveAspectRatio="none" style={{ overflow: "visible" }}>
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            <rect x={x} y={y} width={bw} height={h} rx={bw / 2} ry={bw / 2}
            ...
            <text x={x + bw / 2} y={y - 6} textAnchor="middle"
              style={{ fontFamily: "var(--mono)", fontSize: 9, fill: isHi ? "var(--fg)" : "var(--fg-dim)", fontWeight: 500 }}>
              {d.value}
            </text>
            <text x={x + bw / 2} y={H - padB + 18} textAnchor="middle"
              style={{ fontFamily: "var(--mono)", fontSize: 10, fill: "var(--fg-dim)", letterSpacing: "0.04em" }}>
              {d.label}
            </text>
          </g>
        );
...
export function SmoothedArea({ data, unit = "", height = 180, pinIdx = null, pinLabel = null, accentVar = "--accent" }) {
  const W = 640, H = height;
  const padL = 32, padR = 24, padT = 18, padB = 28;
  const max = Math.max(...data.map(d => d.v)) * 1.15 || 1;
  const min = 0;
  const step = (W - padL - padR) / Math.max(data.length - 1, 1);
  ...
  const gradId = `area-${accentVar.replace(/[^a-zA-Z0-9]/g, "")}`;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={height} preserveAspectRatio="none" style={{ overflow: "visible" }}>
      ...
      {linePath && (
        <path d={linePath} fill="none" stroke={`var(${accentVar})`} strokeWidth="2.2"
      ...
      {pin && (
        <g>
          ...
          <g transform={`translate(${pin.x + 10}, ${pin.y - 22})`}>
            <rect x="0" y="0" rx="6" ry="6" width="84" height="34" fill="var(--bg-card)" stroke="var(--line)" />
...
export function Donut({ segments, size = 180, thickness = 22, centerValue, centerLabel }) {
      ...
      {segments.map((s, i) => {
        ...
        return (
          <path key={i}
            ...
            fill="none" stroke={`url(#donut-${i})`} strokeWidth={thickness} strokeLinecap="round"
...
export function SparkBars({ values, highlight = null, labels }) {
...
export function MultilineChart({ series, labels, height = 220, accentSeries = 0 }) {
  const W = 680, H = height;
  const padL = 36, padR = 16, padT = 16, padB = 26;
  const allVals = series.flatMap(s => s.data.filter(v => v != null));
  const max = allVals.length > 0 ? Math.max(...allVals) * 1.1 : 100;
  const min = allVals.length > 0 ? Math.min(...allVals, 0) : 0;
  const step = (W - padL - padR) / Math.max(labels.length - 1, 1);

  const toPts = (arr) => arr.map((v, i) => ({
    x: padL + i * step,
    y: padT + (H - padT - padB) * (1 - ((v ?? min) - min) / (max - min || 1)),
  }));
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={height} preserveAspectRatio="none" style={{ overflow: "visible" }}>
      ...
      {[0, 0.25, 0.5, 0.75, 1].map(g => (
        <g key={g}>
          <line x1={padL} y1={padT + (H - padT - padB) * g} x2={W - padR} y2={padT + (H - padT - padB) * g}
            stroke="var(--line-soft)" strokeDasharray="2 4" />
          <text x={padL - 8} y={padT + (H - padT - padB) * g + 3} textAnchor="end"
      ...
      {labels.map((l, i) => (
        <text key={i} x={padL + i * step} y={H - 8} textAnchor="middle"
          style={{ fontFamily: "var(--mono)", fontSize: 10, fill: "var(--fg-dim)" }}>
          {l}
        </text>
      ))}
